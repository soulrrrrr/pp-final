...
__global__ void md5(size_t initial_len, uint8_t *ans, int *val)
{
    if (blockDim.x * blockIdx.x + threadIdx.x >= 100000000)
        return;
    ...
    *val = blockDim.x * blockIdx.x + threadIdx.x;
}
